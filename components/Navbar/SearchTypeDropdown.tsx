...
export default function AccountDropdown({ selectedType, setSelectedType }) {
  return (
    <Listbox as={"div"} value={selectedType} onChange={setSelectedType}>
      {({ open }) => (
        <div
          className={`relative inline-block bg-cool-gray-200 ${
        ...
        >
          ...
          {open && (
            <Listbox.Options className="absolute origin-top-right px-2 py-2 bg-white shadow-xl border rounded-b-lg rounded-tr-lg z-50">
              {searchTypeOptions.map((searchTypeOption, index) => (
                <Listbox.Option
                  key={index}
                  value={searchTypeOption}
                  className="px-4 py-1 hover:bg-blue-500 rounded hover:text-white"
                >
